...
**Status:** IMPLEMENTED · steps 1–6 complete, corrected across three adversarial-review rounds · executables resolve through a closed toolchain authority (no ambient PATH, no executable-derived directory prefix; exact-file grants; content-digest verified before execution) · target validator identity binds executable + toolchain + trusted-base scripts (no argv-only fallback) · the public evaluation path is supervised by a hard whole-evaluation deadline in a separate worker process · admission requires an externally-supplied pinned key + attestation over a clean frozen commit, with branded admission/disposition outcomes and a source architecture fence · a runtime control-surface census executes a proof per control and closes a registration-gated ledger · contract authorization is verified against a base-authoritative trust root · **subject is honestly FOUNDATION_ONLY** (no external key pinned in the public build) · closeout: [`closeout-policy-pack-promotion-kernel-v1.md`](closeout-policy-pack-promotion-kernel-v1.md) · 2026-08-26.
