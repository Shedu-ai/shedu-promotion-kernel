...
test("no production file imports a network module or model SDK", () => {
  ...
  for (const file of files) {
    ...
    if (file.endsWith("/sandbox.mjs") || file.endsWith("/control-proofs.mjs")) {
      // These modules embed net references inside CHILD probe SCRIPTS (strings
      // executed under the sandbox) to DEMONSTRATE that the sandbox denies
      // network access. The module itself must still have no top-level
      // network import.
