...
> **Status: FOUNDATION_ONLY (honest).** The full promotion pipeline is implemented — compiler, mandatory integrity packs, OS-sandboxed exact-argv execution (default-deny reads confined to the candidate/base materializations, network denied, filesystem read-only, fork denied; fail-closed when isolation is unavailable, e.g. nested sandboxes), a monotonic evaluation-wide deadline, cumulative-artifact/output/process ceilings, per-phase command scheduling, containment/identity/authority/evidence halts with explicit skip records, an override-free disposition reducer, content-addressed evidence, fingerprint-bound mechanism activation, offline-verifiable and optionally Ed25519-signed receipts, and a control-surface census. Elevation to `EXPERIMENTAL` requires BOTH recomputed conformance invariants AND a detached attestation signed by an externally-pinned key that binds the exact kernel commit. **No such key is pinned in this public build, so the probe honestly reports `FOUNDATION_ONLY`** and the promotion entrypoint is refused — the elevation machinery is real and tested, but cannot be satisfied by editing the subject's own evidence. The sandbox backend currently exists for macOS (`sandbox-exec`); other platforms fail closed. `maxProcesses` is fixed at 1 (fork denial); multi-process test runners are a known limitation and keep pilot blocked rather than weakening isolation.
